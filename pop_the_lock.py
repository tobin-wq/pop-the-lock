...
def play_level(stdscr, level, direction, ring, high_score):
    ...
    while True:
        ...
        ch = stdscr.getch()
        if ch == ord(" "):
            # Tolerance reflects the visual width of the sprites: the ball is
            # ~8 chars wide, peg ~3 wide, ring has ~1.7 chars per stop, so a
            # 2-stop window roughly matches "the ball is touching the peg".
            tol = 2
